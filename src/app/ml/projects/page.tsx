...
export default function MLProjectsPage() {
  const { completedItems } = useProgress('ml');
  ...
  const completedProjects = capstoneProjects.filter(project => project.completed);
  const upcomingProjects = capstoneProjects.filter(project => !project.completed);
      ...
      <div className="container-page">
          ...
          <div className="flex items-center gap-3">
            ...
            {totalProjects > 0 && (
              <div className="flex-1 max-w-xs h-2 rounded-full bg-[#F2F1EE] dark:bg-[#232321] overflow-hidden">
                <div
                  className="h-full bg-[#5D8E72] transition-all duration-300"
        ...
        {completedProjects.length > 0 ? (
          <div className="space-y-4 mb-8">
            ...
            {completedProjects.map(project => (
              <div key={project.id} className="bg-white dark:bg-[#1A1A18] rounded-xl p-5 border border-[#E5E4DF] dark:border-[#2C2B28]">
                <div className="flex items-start gap-3">
                  <Trophy className="w-5 h-5 text-[#5D8E72] dark:text-[#7AAE8E] mt-0.5 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-medium text-[#4F7F64] dark:text-[#7AAE8E] bg-[#E2EDE6] dark:bg-[#1C2820] px-2 py-0.5 rounded">
                        {project.phase}
                      </span>
                      <span className="text-xs text-[#8A8A86] dark:text-[#686664]">{project.phaseTitle}</span>
                    </div>
                    <h3 className="text-sm font-semibold text-[#1A1A1A] dark:text-[#E8E7E4] mb-1">{project.title}</h3>
          ...
          <div className="bg-white dark:bg-[#1A1A18] rounded-xl p-8 border border-[#E5E4DF] dark:border-[#2C2B28] text-center mb-8">
            <Trophy className="w-12 h-12 text-[#5D8E72] dark:text-[#7AAE8E] mx-auto mb-4" />
            <h3 className="text-sm font-semibold text-[#1A1A1A] dark:text-[#E8E7E4] mb-2">No Completed Projects Yet</h3>
        ...
        {upcomingProjects.length > 0 && (
          <div className="space-y-4">
            <h2 className="text-base font-semibold text-[#52524E] dark:text-[#9E9C98] mb-4">Upcoming Projects</h2>
            {upcomingProjects.map(project => (
              <div key={project.id} className="bg-[#F2F1EE] dark:bg-[#232321] rounded-xl p-5 border border-[#E5E4DF] dark:border-[#2C2B28] opacity-70">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-sm font-semibold text-[#52524E] dark:text-[#9E9C98] mb-1">{project.title}</h3>
                    <p className="text-xs text-[#8A8A86] dark:text-[#686664] leading-relaxed">{project.description}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <footer className="text-center mt-10 pt-6 border-t border-[#E5E4DF] dark:border-[#2C2B28]">
          <Link href="/ml" className="inline-flex items-center gap-2 text-sm text-[#8A8A86] dark:text-[#686664] hover:text-[#52524E] dark:hover:text-[#9E9C98] transition-colors">
