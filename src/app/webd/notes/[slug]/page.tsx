...
function getNotes(): Note[] {
  const notesDir = path.join(process.cwd(), 'public', 'data', 'lectures', 'webd');
  const notes: Note[] = [];
...
function getNoteContent(file: string): string {
  const filePath = path.join(process.cwd(), 'public', 'data', 'lectures', 'webd', file);
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  return getLectureMetadata('webd', (await params).slug);
}

export default function WebDNotePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // For HTML lectures, content is loaded via iframe (public URL); skip the read.
  const content = note.kind === 'html' ? '' : getNoteContent(note.file);

  return <><SeoJsonLd data={lectureJsonLd('webd', slug)} /><NoteContent note={note} content={content} /></>;
}
