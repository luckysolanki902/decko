...
function getNotes(): NoteItem[] {
  ...
  phases.forEach(phaseFolder => {
    ...
    files.forEach(file => {
      ...
      const revisionMatch = file.match(/^phase\d+-revision\.(md|html)$/);
      if (revisionMatch) {
        ...
        if (idMatch) {
          notes.push({
            ...
            title: titleMatch ? titleMatch[1].replace(/<[^>]+>/g, '').replace(/\*\*/g, '').trim() : `Phase ${phaseNumber} Revision`,
            duration: 'Quick Read',
            day: 'Revision',
            phaseNumber,
            phaseTitle,
...
function getQuizPhases(): Set<string> {
  const quizDir = path.join(process.cwd(), 'public', 'data', 'quizzes', 'webd');
...
export default function WebDNotesPage() {
  ...
  return (
    <NotesPageClient
      ...
      storageKey="webd-current-phase"
      breadcrumbs={[
        { label: 'Web Dev', href: '/webd' },
        { label: 'Notes' },
      ]}
      notesHref="/webd/notes"
      revisionHrefPrefix="/webd/revision/phase"
    />
  );
}
