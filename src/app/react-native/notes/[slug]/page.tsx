...
function allNotes(): Note[] {
  const root = path.join(process.cwd(),'public','data','lectures','reactnative'); const result: Note[]=[];
...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){return getLectureMetadata('reactnative',(await params).slug);}
export default function ReactNativeNotePage({params}:{params:Promise<{slug:string}>}){const {slug}=use(params);const note=allNotes().find(item=>item.id===slug);if(!note)return notFound();const content=fs.readFileSync(path.join(process.cwd(),'public','data','lectures','reactnative',note.file),'utf8');return <><SeoJsonLd data={lectureJsonLd('reactnative',slug)}/><NoteContent note={note} content={content}/></>;}
